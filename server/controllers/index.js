...
const userRegister = async (ctx) => {
  try {
    ...
    // 检查手机号是否已注册
    const checkUser = await allServices.query(
...
const saveWrongQuestion = async (ctx) => {
  try {
    ...
    const { subject, chapter, content, errorType, yourAnswer } =
      ctx.request.body;
    ...
    // 查询知识点 ID
    const knowledgePointSql =
      "SELECT id FROM knowledge_points WHERE subject_id = ? AND name = ?";
    const knowledgePointResult = await allServices.query(knowledgePointSql, [
      subjectId,
      chapter,
    ]);
    ...
    if (knowledgePointResult.length === 0) {
      ...
      const insertKnowledgeResult = await allServices.query(
        insertKnowledgeSql,
        [subjectId, chapter]
      );
      knowledgePointId = insertKnowledgeResult.insertId;
    } else {
    ...
    `;
    const insertResult = await allServices.query(insertSql, [
      studentId,
      subjectId,
      knowledgePointId,
      errorType,
    ...
    ctx.body = {
      code: 200,
      data: {
        id: insertResult.insertId || insertResult[0]?.insertId,
      },
      message: "保存成功",
    };
  } catch (error) {
...
const addToReviewPlan = async (ctx) => {
  try {
    ...
    const checkSql = `
        ...
        `;
    const checkResult = await allServices.query(checkSql, [
      studentId,
      questionId,
      assignmentDate,
    ]);
...
async function addYesterdayQuestionsToToday(studentId) {
  try {
    ...
    const questionsToAdd = await allServices.query(findSql, [
      studentId,
      yesterdayStr,
      studentId,
      today,
    ...
    if (questionsToAdd.length > 0) {
      const values = questionsToAdd
        .map((q) => `(${studentId}, ${q.id}, '${today}')`)
        .join(",");
      ...
      console.log(
        `已自动添加${questionsToAdd.length}道昨天的错题到今天的复习计划`
      );
    }
  } catch (error) {
...
const updateQuestionAI = async (ctx) => {
  ...
  try {
    // 更新数据库
    const result = await allServices.query(
      // 修复这里
